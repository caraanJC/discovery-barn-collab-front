...
const TaskItem = (props) => {
    ...
    const childProgramSelected = useSelector(
        (state) => state.childProgramSelected
    );
    ...
    const handleOnFileUpload = () => {
        ...
        const storageRef = ref(
            storage,
            `${student.split(' ').join('')}/${taskTitle}/${fileToUpload.name}`
        );
        ...
        uploadFile.on(
            ...
            () => {
                getDownloadURL(uploadFile.snapshot.ref).then((url) => {
                    ...
                    axios
                        .post(
                            `http://localhost:8000/api/students/${studentID}/submissions/upload`,
                            {
                                file_path: url,
                        ...
                        .then((res) => {
                            axios
                                .get(
                                    `http://localhost:8000/api/students/${studentID}/submissions/getSubmissions`
                                )
                                .then((res) => {
                                    dispatch({
    ...
    useEffect(() => {
        if (childSelected !== '') {
            axios
                .get(
                    `http://localhost:8000/api/programs/${childProgramSelected}/getTasks`
                )
                .then((res) => {
        ...
        const targetProgram = schoolPrograms.filter(
            (program) => program._id === childProgramSelected
        );
        const targetChild = parentChildren.filter(
            (child) => child._id === childSelected
        );
        const targetTask = programTasks.filter(
            (task) => task.title === taskTitle
        );
        setDescription(formatParagraph(targetTask[0].description));
        setDueDate(formatDateString(targetTask[0].deadline));
        setDueDateRaw(toNormalTime(targetTask[0].deadline));
            ...
            <div className='container-fluid mt-4'>
                <div className='row'>
                    ...
                    <div className='col-xl-8 col-lg-7 col-md-12 col-sm-12 mb-3'>
                        <div className='mt-4'>
                            <h2 className='text-green task-view-title'>
                                {taskTitle}
                            </h2>
                            <div className='task-view-duedate'>
                                Due Date: {dueDate}
                            </div>
                        </div>
                        <hr />
                            ...
                            <div className='submissions-section-body d-flex flex-nowrap flex-column justify-content-between'>
                                <div className='d-flex flex-wrap flex-row text-center align-items-center justify-content-center'>
                                    ...
                                    <div className='submission-info'>
                                        <p className='submission-info-student'>
                                            {student}
                                        </p>
                                        <p className='submission-info-program'>
                                            {program}
                                        </p>
                                    </div>
                                </div>
                                <div className='submission-uploaded-files'>
                                    {!studentSubmissions.find(
                                        (submission) =>
                                            submission.task_title === taskTitle
                                    ) ? (
                                ...
                                {!studentSubmissions.find(
                                    ...
                                    <button
                                        className='myButton btn-success btn btn-block submission-upload-button'
