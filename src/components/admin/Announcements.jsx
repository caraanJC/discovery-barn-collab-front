...
const Announcements = (props) =>{
    ...
    const handleAddEditTableRow = () => {
        if (title.trim() === '') {
            ...
            if (targetId === 'ADD') {
                axios
                    .post('http://localhost:8000/api/announcements', rowData)
                    .then((res) => {
                        if (res.data.success) {
                            setShowModalFlag(false);
                            clearAddEditModal();
                ...
                axios
                    .put(`http://localhost:8000/api/announcements/${targetId}`,rowData)
                    .then((res) => {
                        if (res.data.success) {
                            setShowModalFlag(false);
                            clearAddEditModal();
        ...
        <>
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title>
                        {targetId === 'ADD' ? 'New Announcement' : 'Edit Announcement'}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    ...
                    <Form.Group className='mb-3'>
                        ...
                        <Form.Control
                            type='text'
                            value={subtitle}
                            onChange={(e) => handleOnInputChange(e, 'subtitle')}
                        />
                    </Form.Group>
                    <Form.Group className="mb-3">
