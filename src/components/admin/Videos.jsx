...
const Videos = (props) => {
	...
	const handleOnFileUpload = () => {
		...
		if (uploadType === '' || uploadType === null) {
			...
			const storageRef = ref(storage, `${directory}/` + randomStr + '__DBP54321__' + fileToUpload.name);
	...
	const handleAddEditVideo = () => {
		if (title.trim() === '') {
			...
			if (targetVideoId === 'ADD') {
				axios.post('http://localhost:8000/api/videos', newVideo).then((res) => {
					if (res.data.success) {
						setShowModalFlag(false);
						clearVideoModal();
						axios.get('http://localhost:8000/api/videos').then((res) => {
				...
				axios.put(`http://localhost:8000/api/videos/${targetVideoId}`, newVideo).then((res) => {
					if (res.data.success) {
						setShowModalFlag(false);
						clearVideoModal();
						axios.get('http://localhost:8000/api/videos').then((res) => {
			...
			<Modal show={showUploadModalFlag} onHide={() => handleUploadModalHidden()} keyboard={false}>
				...
				<Modal.Body>
					...
					<Form.Group className='mb-3'>
						...
						<Form.Select value={uploadType} onChange={(e) => handleOnChangeFileType(e)}>
							<option value=''></option>
